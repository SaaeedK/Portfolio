...
async function main() {
  ...
  const roleBlock = (company, location, title, dates) => {
    ...
    drawText(`${location}  |  ${title}  |  ${dates}`, { size: 8.5, color: COLORS.muted, lineGap: 2 });
  };
  ...
  // --- Header ---
  drawText('Saaeed Khan', { size: 22, bold: true, lineGap: 1 });
  drawText('U.S. Security Clearance: Eligible', { size: 9, bold: true, color: COLORS.accent, lineGap: 2 });
  drawLine(MARGIN, PAGE.width - MARGIN);
