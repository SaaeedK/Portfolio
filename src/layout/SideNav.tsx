...
export const SideNav = () => {
  return (
    <aside
      className="hidden lg:flex h-screen w-64 fixed left-0 top-0 z-40 flex-col bg-surface-container/30 backdrop-blur-xl border-r border-primary-fixed/20 pt-24 pb-8 justify-between group"
      aria-label="Primary"
    >
      <div>
        <div className="px-6 py-4 flex flex-col gap-2 border-b border-primary-fixed/10 pb-6 mb-4">
          <div className="flex items-center gap-4 group-hover:translate-x-1 duration-300">
            <div className="w-10 h-10 rounded-full bg-surface-variant border border-primary-fixed/40 flex items-center justify-center overflow-hidden">
              <ShieldCheck className="text-primary-fixed" size={20} aria-hidden />
            </div>
            <div className="flex flex-col min-w-0">
              <span className="font-mono text-xs font-bold text-secondary-fixed truncate">{site.displayHandle}</span>
              <span className="font-mono text-[9px] text-on-surface-variant tracking-wider">Portfolio · static site</span>
            </div>
          </div>
          <NavLink
            to="/labs"
        ...
        <nav className="flex flex-col gap-1" aria-label="Sections">
          {[
            ...
            >
              <item.icon size={18} className="group-hover/item:scale-110 transition-transform" aria-hidden />
